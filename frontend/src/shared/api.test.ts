...
describe('apiFetch', () => {
  it('sends cookie credentials and parses json', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response('{"ok":true}', { status: 200, headers: { 'Content-Type': 'application/json' } }));
    vi.stubGlobal('fetch', fetchMock);
    (globalThis as Record<string, unknown>).__BOOT__ = { origin: 'http://x', base_prefix: '', authenticated: true, version: '', serial: '' };
