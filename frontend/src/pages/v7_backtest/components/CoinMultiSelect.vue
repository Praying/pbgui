  ...
  <div class="form-group">
    <label :data-tip="tip || undefined">
      ...
      <span class="ms-label-actions">
        ...
        <button v-if="model.length" type="button" class="ms-clear-btn" :title="t('v7backtest.clearAll')" :aria-label="t('v7backtest.clearAll')" style="border: 0; background: transparent; font: inherit; cursor: pointer" @click="clear"><PbIcon :icon="PhX" /></button>
      </span>
    </label>
    <div :id="id" class="ms-wrap" @focusin="open = true" @focusout="open = false">
      <span v-for="value in model" :key="value" class="ms-tag" :class="{ 'ms-tag-all': value === 'all' }">
        ...
        <button type="button" class="ms-x" :aria-label="`Remove ${value === 'all' ? 'all' : display(value)}`" :title="`Remove ${value === 'all' ? 'all' : display(value)}`" style="border: 0; background: transparent; padding: 0; font: inherit; cursor: pointer" @mousedown.prevent @click.stop="toggle(value)"><PbIcon :icon="PhX" /></button>
      </span>
