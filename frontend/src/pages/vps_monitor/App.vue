    ...
    <div class="flex min-h-0 flex-1">
      <section class="flex min-w-0 flex-1 flex-col p-[var(--page-padding)]">
        ...
        <div v-else-if="activeTab === 'instances'" class="min-h-0 flex-1 overflow-auto">
          <div class="mb-3.5 flex flex-wrap gap-3.75 text-[0.85rem] text-primary"><span class="inline-flex items-center gap-1.5"><span id="instance-server-label">Server</span> <SelectRoot v-model="instanceServerFilter"><SelectTrigger class="w-auto min-w-[110px]" aria-labelledby="instance-server-label"><span>{{ instanceServerFilter }}</span></SelectTrigger><SelectContent><SelectItem value="All">All</SelectItem><SelectItem v-for="host in hosts" :key="host" :value="host">{{ host }}</SelectItem></SelectContent></SelectRoot></span><span class="inline-flex items-center gap-1.5"><span id="instance-version-label">{{ t('sysmon.version') }}</span> <SelectRoot v-model="instanceVersionFilter"><SelectTrigger class="w-auto min-w-[90px]" aria-labelledby="instance-version-label"><span>{{ instanceVersionFilter }}</span></SelectTrigger><SelectContent><SelectItem value="All">All</SelectItem><SelectItem value="7">7</SelectItem><SelectItem value="8">8</SelectItem><SelectItem value="V7">V7</SelectItem><SelectItem value="V8">V8</SelectItem></SelectContent></SelectRoot></span><label class="flex items-center gap-1.75"><Checkbox v-model="instanceErrorsOnly" /> {{ t('sysmon.onlyWithErrors') }}</label><label class="flex items-center gap-1.75"><Checkbox v-model="instanceShowOther" /> {{ t('sysmon.showOther') }}</label></div>
