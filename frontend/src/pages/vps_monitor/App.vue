...
function metricLabel(metric: string): string {
  const labels: Record<string, string> = { cpu: t('sysmon.cpu'), memory: t('sysmon.ram'), disk: t('sysmon.disk'), swap: t('sysmon.swap') };
  return labels[metric] || metric;
}
...
/** Status token → i18n key, covering connection, service and instance states. */
const STATUS_LABEL_KEYS: Record<string, string> = {
  running: 'sysmon.running',
  stopped: 'sysmon.stopped',
  restarting: 'sysmon.restarting',
  disabled: 'sysmon.disabled',
  connected: 'sysmon.connected',
  connecting: 'sysmon.connecting',
  lost: 'sysmon.disconnected',
  synced: 'sysmon.synced',
  outdated: 'sysmon.outdated',
  pending: 'sysmon.pending',
  ok: 'sysmon.ok',
  stale: 'sysmon.stale',
  missing: 'sysmon.missing',
  error: 'common.error',
};

/** Unknown status tokens fall through to their raw value. */
...
function onKeydown(event: KeyboardEvent): void {
  ...
  if (historyModal.value) historyModal.value = null;
  else if (resultModal.value) closeResult();
}
        ...
        <div v-if="activeTab === 'dashboard'" class="min-h-0 flex-1 overflow-auto">
          ...
          <article v-for="host in hosts" :key="host" class="mb-2.5 overflow-hidden rounded-lg border border-border-default bg-card">
            <header class="flex max-[780px]:flex-wrap cursor-pointer items-center gap-2.25 px-3.25 py-2.75 hover:bg-accent/8" role="button" tabindex="0" :aria-expanded="!isHostCollapsed(host)" @click="toggleHost(host)" @keydown.enter.prevent="toggleHost(host)" @keydown.space.prevent="toggleHost(host)">
              ...
              <span class="rounded-full px-1.75 py-0.75 text-xs" :class="badgeStatusClass(connectionMap[host]?.status)">{{ statusLabel(connectionMap[host]?.status) }}</span>
            ...
            <div v-if="!isHostCollapsed(host)" class="border-t border-border-default p-3">
              ...
              <div v-if="connectionMap[host]?.status !== 'connected'" class="pt-3.5 whitespace-pre-wrap break-words text-primary">{{ statusLabel(connectionMap[host]?.status) }}<span v-if="connectionMap[host]?.error">: {{ connectionMap[host]?.error }}</span></div>
