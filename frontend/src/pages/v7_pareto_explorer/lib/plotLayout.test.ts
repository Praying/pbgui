...
describe('darkPlotLayout (:2580-2644)', () => {
  it('applies the dark theme defaults to an empty layout', () => {
    ...
    expect(layout.paper_bgcolor).toBe('#16141a');
    expect(layout.plot_bgcolor).toBe('#16141a');
    expect(layout.font).toEqual({ color: '#e9e5ee' });
  ...
  it('darkens xaxis/yaxis without dropping caller axes config', () => {
    ...
    expect(layout.xaxis).toEqual({ color: '#e9e5ee', gridcolor: 'rgba(255,255,255,0.12)', zerolinecolor: 'rgba(255,255,255,0.18)', title: { text: 'adg' }, range: [0, 1] });
    expect(layout.yaxis).toEqual({ color: '#e9e5ee', gridcolor: 'rgba(255,255,255,0.12)', zerolinecolor: 'rgba(255,255,255,0.18)', type: 'log' });
  });
  ...
  it('darkens scene axes and keeps their titles', () => {
    ...
    expect(scene.bgcolor).toBe('#16141a');
    expect(scene.camera).toEqual({ eye: 1 });
    expect(scene.xaxis).toEqual({ backgroundcolor: '#16141a', gridcolor: 'rgba(255,255,255,0.12)', color: '#e9e5ee', title: 'x' });
  });
  ...
  it('darkens polar axes and colorbar fonts', () => {
    ...
    expect(polar.bgcolor).toBe('#16141a');
    expect(polar.radialaxis).toEqual({ color: '#e9e5ee', gridcolor: 'rgba(255,255,255,0.16)', linecolor: 'rgba(255,255,255,0.22)', tickfont: { color: '#e9e5ee' }, range: [0, 1] });
    const coloraxis = layout.coloraxis as Record<string, Record<string, unknown>>;
    expect(coloraxis.colorbar!.thickness).toBe(20);
    expect(coloraxis.colorbar!.title).toEqual({ text: 'adg', font: { color: '#e9e5ee' } });
    expect(coloraxis.colorbar!.tickfont).toEqual({ color: '#e9e5ee' });
  });
  ...
  it('darkens annotation fonts without mutating the input annotation', () => {
    ...
    expect(out[0]!.font).toEqual({ color: '#e9e5ee', size: 12 });
...
describe('previewPlotLayout (:2914-2923, :2928-2937)', () => {
  it('lifts the legend above the plot and enforces a 750px minimum height', () => {
    ...
    expect(layout.legend).toEqual({ font: { color: '#e9e5ee' }, orientation: 'h', x: 0, xanchor: 'left', y: 1.14, yanchor: 'bottom' });
