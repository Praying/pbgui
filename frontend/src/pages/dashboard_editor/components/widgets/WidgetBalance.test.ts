...
describe('WidgetBalance', () => {
  ...
  it('renders the table headers and rows (renderBalanceRows, render.js:428-449)', async () => {
    ...
    expect(tds[3]!.attributes('style')).toContain('rgb(34, 197, 94)'); /* #22c55e */
    ...
    expect(tds2[4]!.find('.db-twe-lbl').attributes('style')).toContain('rgb(239, 68, 68)'); /* #ef4444 */
  });
  ...
  it('updates rows in place when the live poll delivers a live payload', async () => {
    ...
    expect(status.attributes('style')).toContain('rgb(34, 197, 94)'); /* #22c55e */
  });
