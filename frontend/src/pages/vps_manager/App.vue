...
function openBotMetric(bot: JsonRecord): void { const botName = String(bot.name || ''); metric.value = { loading: true, data: null, bot: botName, name: botName, error: '' }; send({ cmd: 'get_metric_history', hostname: hostname.value, bot_name: botName, metric: 'cpu' }); setModal('history', metric.value); }
