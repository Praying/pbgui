...
const logSizeKb = ref<number | string>(50);
...
function noticeKindClass(kind: string): string {
  ...
  if (kind === 'err') return 'border border-danger/30 bg-danger/10 text-danger-soft';
  return '';
}
...
/* manager-btn variant mapping (the former .manager-btn .primary/.warn/
   .danger/.small and .manager-file-row rules) is now carried by the ui/
   Button variants at each call site: base→default, primary→info,
   warn→warning, danger→danger, small→size="sm", file→default +
...
function sortMark(field: string): string { return overviewSort.value.field === field ? (overviewSort.value.direction === 'asc' ? ' \u25B2' : ' \u25BC') : ''; }
function sortAria(field: string): 'ascending' | 'descending' | 'none' { return overviewSort.value.field === field ? (overviewSort.value.direction === 'asc' ? 'ascending' : 'descending') : 'none'; }
...
/* Closed-state trigger labels for the reka listboxes (options mount lazily,
   so the trigger renders the label from the model — playbook pattern). */
function deployActionLabel(command: unknown): string {
  const item = deployActions.value.find((entry: JsonRecord) => String(entry.command) === String(command ?? ''));
  return item ? String(item.command_text || actionText(item.command)) : clean(command);
}
function commitOptionLabel(value: unknown): string {
  const commit = clean(value);
  if (!commit) return 'HEAD';
  const list = branchCommits({ ...currentBranches.value, ...branchState.value }, branchState.value.sourceBranch || branchState.value.branch || currentBranches.value.current_branch || 'main');
  const match = list.find((entry: JsonRecord) => String(entry.full || entry.hash || entry) === commit);
  return match ? String(match.short || match.hash || match.full || match) : commit;
}
      ...
      <section class="min-w-0 flex-1 overflow-auto p-[var(--page-padding)]">
        ...
        <section v-if="view === 'overview'" class="grid gap-3.5">
          <article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel">
            ...
            <div class="overflow-auto p-3.25"><table class="manager-table w-full border-collapse text-[0.78rem]"><thead><tr><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-pointer" data-sort="hostname" :aria-sort="sortAria('hostname')" @click="sortOverview('hostname')">{{ t('vpsmgr.hostname') }}{{ sortMark('hostname') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.status">{{ t('vpsmgr.status') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.ip" @click="sortOverview('ip')" :aria-sort="sortAria('ip')">{{ t('vpsmgr.ipLabel') }}{{ sortMark('ip') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.cpu">CPU</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.memory">RAM</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.disk">{{ t('vpsmgr.disk') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.pbgui">PBGui</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.pb7">PB7</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.pb8">PB8</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.updates">{{ t('vpsmgr.updates') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default" v-if="visibleColumns.actions">{{ t('vpsmgr.action') }}</th></tr></thead><tbody><tr class="cursor-pointer" v-for="row in sortedRows()" :key="row.hostname" :data-row-host="row.hostname" :class="{ selected: selectedHosts.includes(String(row.hostname || '')) }" @pointerdown="startOverviewDrag(row, $event)" @pointerenter="moveOverviewDrag(row)" @click="handleOverviewRowClick(row)"><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(row.name || row.hostname) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.status"><span class="inline-block rounded-full px-1.75 py-0.5 text-[0.72rem]" :class="statusClass(row.online ? 'online' : 'offline')">{{ row.online ? t('vpsmgr.online') : t('vpsmgr.offline') }}</span><small v-if="row.ssh_host_key_status"> {{ display(row.ssh_host_key_status) }}</small></td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.ip">{{ display(row.ip) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.cpu">{{ formatPercent(row.cpu) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.memory">{{ formatPercent(row.memory_percent ?? row.memory?.percent) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.disk">{{ formatPercent(row.disk_percent ?? row.disk?.percent) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.pbgui">{{ display(row.pbgui) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.pb7">{{ display(row.pb7_branch) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.pb8">{{ display(row.pb8_branch) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.updates">{{ display(row.updates) }}<span v-if="row.task_current_label" class="block text-secondary leading-[1.45]">{{ row.task_current_label }}</span></td><td class="border-b border-border-default px-1.75 py-2 text-left align-top" v-if="visibleColumns.actions"><Button v-if="row.nav === 'vps'" type="button" variant="default" size="sm" data-action="select-vps" :data-host="row.hostname" @pointerdown.stop @click.stop="selectHost(row)">{{ t('common.view') }}</Button><Button v-else type="button" variant="default" size="sm" @pointerdown.stop @click.stop="selectHost(row)">{{ t('common.view') }}</Button></td></tr></tbody></table><LoadingSkeleton v-if="!stateLoaded" :label="t('common.loading')" /><div v-else-if="!rows.length" class="p-4.5 text-center text-secondary">{{ t('common.noData') }}</div></div>
          </article>
        ...
        <section v-else-if="view === 'add-vps'" class="grid gap-3.5 grid-cols-[repeat(auto-fit,minmax(340px,1fr))] max-[900px]:grid-cols-1"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.addVpsTitle') }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="open-existing-import" @click="openExistingImport">{{ t('vpsmgr.importExisting') }}</Button><Button type="button" variant="default" data-action="open-cluster-import" @click="openClusterImport">{{ t('vpsmgr.importClusterNodes') }}</Button></div></div><div class="p-3.25 grid gap-2.5"><div class="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2.5 max-[680px]:grid-cols-1"><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.hostname') }}<Input data-field="add-hostname" :model-value="addForm.hostname" @update:model-value="setAddField('hostname', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsIpv4') }}<Input data-field="add-ip" :model-value="addForm.ip" @update:model-value="setAddField('ip', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserName') }}<Input data-field="add-user" :model-value="addForm.user" @update:model-value="setAddField('user', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserPassword') }}<Input type="password" data-field="add-user-password" :model-value="addForm.user_pw" @update:model-value="setAddField('user_pw', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.installPathLabel') }}<Input data-field="add-install-dir" :model-value="addForm.install_dir" @update:model-value="setAddField('install_dir', $event)" /></label><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="add-init-method-label">{{ t('vpsmgr.initMethod') }}</span><SelectRoot :model-value="addForm.init_methode" @update:model-value="setAddField('init_methode', $event)"><SelectTrigger data-field="add-init-method" aria-labelledby="add-init-method-label"><span>{{ addForm.init_methode }}</span></SelectTrigger><SelectContent><SelectItem v-for="method in config.init_methods || ['root', 'user']" :key="method" :value="method">{{ method }}</SelectItem></SelectContent></SelectRoot></div><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'root'">{{ t('vpsmgr.initialRootPassword') }}<Input type="password" data-field="add-initial-root-password" :model-value="addForm.initial_root_pw" @update:model-value="setAddField('initial_root_pw', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'root'">{{ t('vpsmgr.rootPasswordLabel') }}<Input type="password" data-field="add-root-password" :model-value="addForm.root_pw" @update:model-value="setAddField('root_pw', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'password'">{{ t('vpsmgr.sudoUser') }}<Input data-field="add-sudo-user" :model-value="addForm.user_sudo" @update:model-value="setAddField('user_sudo', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'password'">{{ t('vpsmgr.sudoPassword') }}<Input type="password" data-field="add-sudo-password" :model-value="addForm.user_sudo_pw" @update:model-value="setAddField('user_sudo_pw', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'private_key'">{{ t('vpsmgr.privateKeyUser') }}<Input data-field="add-private-key-user" :model-value="addForm.private_key_user" @update:model-value="setAddField('private_key_user', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="addForm.init_methode === 'private_key'">{{ t('vpsmgr.privateKeyFile') }}<Input data-field="add-private-key-file" :model-value="addForm.private_key_file" @update:model-value="setAddField('private_key_file', $event)" /><Button type="button" variant="default" size="sm" class="justify-self-start" @click="browsePrivateKey">{{ t('vpsmgr.browse') }}</Button></label><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="add-swap-label">{{ t('vpsmgr.swap') }}</span><SelectRoot :model-value="addForm.swap" @update:model-value="setAddField('swap', $event)"><SelectTrigger data-field="add-swap" aria-labelledby="add-swap-label"><span>{{ addForm.swap }}</span></SelectTrigger><SelectContent><SelectItem v-for="swap in config.swap_options || ['0', '2G']" :key="swap" :value="swap">{{ swap }}</SelectItem></SelectContent></SelectRoot></div><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="add-runtime-profile-label">{{ t('vpsmgr.runtimeProfile') }}</span><SelectRoot :model-value="addForm.runtime_profile" @update:model-value="setAddField('runtime_profile', $event)"><SelectTrigger data-field="add-runtime-profile" aria-labelledby="add-runtime-profile-label"><span>{{ addForm.runtime_profile === 'pb8' ? 'PB8 Live only' : addForm.runtime_profile === 'pb7_pb8' ? 'PB7 + PB8' : 'PB7' }}</span></SelectTrigger><SelectContent><SelectItem value="pb7">PB7</SelectItem><SelectItem value="pb8">PB8 Live only</SelectItem><SelectItem value="pb7_pb8">PB7 + PB8</SelectItem></SelectContent></SelectRoot></div><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.firewallSshPort') }}<Input type="number" data-field="add-firewall-port" :model-value="addForm.firewall_ssh_port" @update:model-value="setAddField('firewall_ssh_port', numberValue($event))" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.allowedSshIps') }}<Input data-field="add-firewall-ips" :model-value="addForm.firewall_ssh_ips" @update:model-value="setAddField('firewall_ssh_ips', $event)" /></label></div><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox :model-value="Boolean(addForm.firewall)" @update:model-value="setAddField('firewall', $event === true)" /> {{ t('vpsmgr.enableFirewall') }}</label><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox :model-value="Boolean(addForm.remove_user)" @update:model-value="setAddField('remove_user', $event === true)" /> {{ t('vpsmgr.removeUserLabel') }}</label><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="detect-public-ip" @click="detectIp">{{ t('vpsmgr.detectPublicIp') }}</Button><Button type="button" variant="default" data-action="check-vps-ready" @click="checkReady">{{ t('vpsmgr.preflight') }}</Button><Button type="button" variant="default" data-action="write-hosts" @click="writeHosts">{{ t('vpsmgr.addHosts') }}</Button><Button type="button" variant="default" data-action="save-add-vps" @click="saveAddVps">{{ t('vpsmgr.save') }}</Button><Button type="button" variant="info" data-action="init-add-vps" :disabled="!addFormReady()" @click="initAddVps">{{ t('vpsmgr.initialize') }}</Button></div></div></article><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.preflight') }}</span></div><div class="p-3.25"><div v-for="(value, key) in addReady" :key="String(key)" class="mb-3 block rounded-[7px] bg-card px-3 py-2.25 whitespace-pre-line" :class="checkStatusClass(value)">{{ key }}: {{ display(value) }}</div><Button v-if="addReady.host_key?.needs_confirmation" type="button" variant="warning" data-action="accept-add-host-key" @click="acceptAddHostKey">{{ display(addReady.host_key?.fingerprint) }} — {{ t('vpsmgr.trustKeyReconnect') }}</Button><pre v-if="publicIp" class="m-0 overflow-auto whitespace-pre-wrap break-words rounded-[5px] bg-page p-2.5 font-mono text-[0.75rem] leading-[1.4] text-primary">{{ publicIp }}</pre></div></article></section>

        <section v-else-if="view === 'deploys-vps-logging'" class="grid gap-3.5 grid-cols-[repeat(auto-fit,minmax(340px,1fr))] max-[900px]:grid-cols-1"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.deploymentsLabel') }}</span></div><div class="p-3.25 grid gap-2.5"><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="deploy-action-label">{{ t('vpsmgr.action') }}</span><SelectRoot v-model="deploySettings.action"><SelectTrigger data-field="deploy-action" aria-labelledby="deploy-action-label"><span :class="deploySettings.action ? undefined : 'text-placeholder'">{{ deployActionLabel(deploySettings.action) }}</span></SelectTrigger><SelectContent><SelectItem v-for="item in deployActions" :key="item.command" :value="item.command">{{ item.command_text || actionText(item.command) }}</SelectItem></SelectContent></SelectRoot></div><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="deploy-mode-label">{{ t('vpsmgr.mode') }}</span><SelectRoot v-model="deploySettings.mode"><SelectTrigger data-field="deploy-mode" aria-labelledby="deploy-mode-label"><span :class="deploySettings.mode ? undefined : 'text-placeholder'">{{ deploySettings.mode }}</span></SelectTrigger><SelectContent><SelectItem v-for="mode in deployModes" :key="mode" :value="mode">{{ mode }}</SelectItem></SelectContent></SelectRoot></div><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox v-model="deploySettings.debug" /> {{ t('vpsmgr.debug') }}</label><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox v-model="deploySettings.reboot_requested" /> {{ t('vpsmgr.rebootAfterUpdate') }}</label><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="save-deploy-settings" @click="saveDeploySettings">{{ t('vpsmgr.save') }}</Button><Button type="button" variant="info" data-action="run-deploy" @click="startDeploy">{{ t('vpsmgr.validateAndStart') }}</Button></div></div></article><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.deploymentHistory') }}</span></div><div class="p-3.25"><div v-if="state.deploys?.progress_rows?.length" class="grid gap-1.75"><div v-for="row in state.deploys.progress_rows" :key="`${row.hostname}-${row.command}`" class="grid gap-1.25 rounded-md border border-border-default bg-card p-2.25"><div><strong>{{ display(row.hostname) }}</strong> — {{ display(row.command_text || row.command) }} — {{ display(row.status) }}</div><div class="h-2 overflow-hidden rounded-full bg-border-strong"><div class="h-full bg-accent" :style="{ width: `${Number(row.task_progress?.percent || row.percent || 0)}%` }"></div></div><small class="text-secondary">{{ display(row.task_current_label || row.reason) }}</small></div></div><div v-if="!deployHistory.length" class="p-4.5 text-center text-secondary">{{ t('common.noData') }}</div><table v-else class="manager-table w-full border-collapse text-[0.78rem]"><thead><tr><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.started') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.action') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.hosts') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.status') }}</th></tr></thead><tbody><tr class="cursor-pointer" v-for="entry in deployHistory" :key="entry.id"><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(entry.started_at) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(entry.command_text || entry.command) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top"><div v-for="host in entry.hostnames || []" :key="host" class="my-0.75 flex min-w-[210px] items-center justify-between gap-1.75"><span>{{ host }}</span><Button type="button" variant="default" size="sm" data-action="open-deploy-log" :disabled="!deployLogFile(entry, host)" @click="openDeployLog(entry, host)">{{ t('vpsmgr.openLog') }}</Button></div></td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(entry.status || entry.result || '—') }}</td></tr></tbody></table></div></article></section>

        <section v-else-if="view === 'settings-vps-logging'" class="grid gap-3.5 grid-cols-[repeat(auto-fit,minmax(340px,1fr))] max-[900px]:grid-cols-1"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.vpsLogging') }}</span></div><div class="p-3.25 grid gap-2.5"><label class="grid gap-1 text-[0.8rem] text-secondary" v-for="service in vpsLogging" :key="service.service">{{ service.service }} (MB)<Input :data-field="`logging-limit-${service.service}`" type="number" :model-value="service.max_mb" @update:model-value="service.max_mb = numberValue($event)" /></label><Button type="button" variant="info" data-action="save-logging-settings" @click="saveLoggingSettings">{{ t('vpsmgr.save') }}</Button><Button type="button" variant="default" data-action="deploy-logging" @click="deployLogging">{{ t('vpsmgr.deployLogging') }}</Button></div></article><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.vpsDeploySettings') }}</span></div><div class="p-3.25"><JsonViewer :data="deploySettings" /></div></article></section>

        <section v-else-if="view.includes('branch')" class="grid gap-3.5"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ currentTarget }} — {{ currentRepo.toUpperCase() }} {{ t('vpsmgr.branch') }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="load-remote-branches" @click="loadRemoteBranches">{{ t('vpsmgr.loadRemoteBranches') }}</Button><Button type="button" variant="default" data-action="load-more-commits" @click="loadMoreCommits">+50</Button><Button type="button" variant="info" data-action="run-branch-action" @click="runBranchAction">{{ t('vpsmgr.update') }}</Button></div></div><div class="p-3.25 grid gap-2.5"><div class="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-2"><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ t('vpsmgr.currentBranch') }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]">{{ display(currentBranches.current_branch) }}</div></div><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ t('vpsmgr.currentCommit') }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]">{{ display(currentBranches.current_commit) }}</div></div></div><div class="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2.5 max-[680px]:grid-cols-1"><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="branch-target-label">{{ t('vpsmgr.targetBranch') }}</span><SelectRoot :model-value="branchState.branch || currentBranches.current_branch || 'main'" @update:model-value="setBranchField('branch', $event)"><SelectTrigger aria-labelledby="branch-target-label"><span>{{ branchState.branch || currentBranches.current_branch || 'main' }}</span></SelectTrigger><SelectContent><SelectItem v-for="branch in (branchState.remoteBranches?.length ? branchState.remoteBranches : branchOptions(currentBranches))" :key="branch" :value="branch">{{ branch }}</SelectItem></SelectContent></SelectRoot></div><label class="grid gap-1 text-[0.8rem] text-secondary" v-if="currentRepo !== 'pbgui'">{{ t('vpsmgr.sourceBranch') }}<Input :model-value="branchState.sourceBranch || branchState.branch || ''" @update:model-value="setBranchField('sourceBranch', $event)" /></label><!-- ui-migration: the legacy <option value="">HEAD</option> has no reka equivalent (empty string is the cleared state); resetting to HEAD stays available via the "use HEAD" button below. --><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="branch-commit-label">{{ t('vpsmgr.targetCommit') }}</span><SelectRoot :model-value="branchState.commit || currentBranches.current_commit || ''" @update:model-value="setBranchField('commit', $event)"><SelectTrigger aria-labelledby="branch-commit-label"><span>{{ commitOptionLabel(branchState.commit || currentBranches.current_commit || '') }}</span></SelectTrigger><SelectContent><SelectItem v-for="commit in branchCommits({ ...currentBranches, ...branchState }, branchState.sourceBranch || branchState.branch || currentBranches.current_branch || 'main')" :key="commit.full || commit.hash || commit" :value="commit.full || commit.hash || commit">{{ commit.short || commit.hash || commit.full || commit }}</SelectItem></SelectContent></SelectRoot></div><div class="grid gap-1 text-[0.8rem] text-secondary" v-if="currentRepo !== 'pbgui'"><span id="branch-remote-name-label">{{ t('vpsmgr.remoteName') }}</span><SelectRoot :model-value="branchState.remoteName || currentBranches.default_remote_name || 'origin'" @update:model-value="setBranchRemoteName(String($event ?? ''))"><SelectTrigger aria-labelledby="branch-remote-name-label"><span>{{ branchState.remoteName || currentBranches.default_remote_name || 'origin' }}</span></SelectTrigger><SelectContent><SelectItem v-for="name in currentBranches.known_remotes || ['origin', 'fork']" :key="name" :value="name">{{ name }}</SelectItem></SelectContent></SelectRoot></div><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.remoteUrl') }}<Input :model-value="branchState.remoteUrl || currentBranches.remote_url || ''" @update:model-value="setBranchField('remoteUrl', $event)" /></label></div><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="load-remote-commits" @click="loadRemoteCommits">{{ t('vpsmgr.loadRemoteCommits') }}</Button><Button type="button" variant="default" @click="setBranchField('commit', '')">{{ t('vpsmgr.useHead') }}</Button><Button v-if="currentRepo !== 'pbgui'" type="button" variant="default" data-action="use-local-branch" @click="useLocalBranchTarget">{{ t('vpsmgr.useLocalBranchTarget') }}</Button><Button v-if="currentRepo !== 'pbgui'" type="button" variant="warning" data-action="use-pinned-upstream" @click="usePinnedUpstream">{{ t('vpsmgr.usePinnedUpstream') }}</Button></div><div v-if="branchState.remoteBranches?.length" class="block text-secondary leading-[1.45]">{{ branchState.remoteBranches.join(', ') }}</div><table class="manager-table w-full border-collapse text-[0.78rem]"><thead><tr><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">Hash</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.messageLabel') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.authorLabel') }}</th></tr></thead><tbody><tr class="cursor-pointer" v-for="commit in branchCommits({ ...currentBranches, ...branchState }, branchState.sourceBranch || branchState.branch || currentBranches.current_branch || 'main')" :key="commit.hash || commit"><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(commit.hash || commit) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(commit.subject || commit.message) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(commit.author) }}</td></tr></tbody></table></div></article></section>

        <section v-else-if="view === 'master' || view === 'vps'" class="grid gap-3.5"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ display(detail?.hostname || hostname || config.master_name) }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button v-if="!isMasterContext" type="button" variant="warning" data-action="setup-vps" @click="setupVps">{{ t('vpsmgr.setup') }}</Button><Button v-if="!isMasterContext" type="button" variant="info" data-action="deploy-vps" @click="runDirectDeploy">{{ t('vpsmgr.deploy') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="cluster-onboard" @click="startClusterOnboard">{{ t('vpsmgr.addToCluster') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="review-host-key" @click="openHostKeyReview">{{ t('vpsmgr.reviewSshHostKey') }}</Button><Button v-if="!isMasterContext" type="button" variant="danger" data-action="delete-vps" @click="requestDelete">{{ t('vpsmgr.deleteVps') }}</Button></div></div><div class="p-3.25"><div v-if="!detail" class="p-4.5 text-center text-secondary">{{ t('common.loading') }}</div><template v-else><div class="grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-2"><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ t('vpsmgr.status') }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]"><span class="inline-block rounded-full px-1.75 py-0.5 text-[0.72rem]" :class="statusClass(detail.status?.online ? 'online' : 'offline')">{{ detail.status?.online ? t('vpsmgr.online') : t('vpsmgr.offline') }}</span></div></div><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ t('vpsmgr.ipLabel') }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]">{{ display(detail.status?.ip || detail.config?.ip) }}</div></div><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">CPU</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]">{{ formatPercent(detail.status?.server_metrics?.cpu?.value || detail.status?.cpu) }}</div></div><div class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ t('vpsmgr.packageStatus') }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]"><Button type="button" variant="default" size="sm" data-action="open-package-updates" data-modal-trigger="package-updates" @click="openPackageUpdates">{{ display(detail.status?.package_status?.upgrades || 0) }}</Button></div></div></div><div class="flex flex-wrap items-center justify-start gap-1.75 my-3.5 rounded-lg bg-page/72 p-2.5"><Button type="button" variant="default" size="sm" data-action="metric-history" @click="loadMetricHistory('cpu')">{{ t('vpsmgr.cpuHistory') }}</Button><Button v-if="isMasterContext" type="button" variant="default" @click="runMasterCommand('master-update-pbgui')">{{ t('vpsmgr.updatePbgui') }}</Button><Button v-if="isMasterContext && hasRuntime('pb7')" type="button" variant="default" @click="runMasterCommand('master-update-pb')">{{ t('vpsmgr.updatePbguiAndPb7') }}</Button><Button v-if="isMasterContext && hasRuntime('pb8')" type="button" variant="default" @click="runMasterCommand('master-update-pbgui-pb8')">{{ t('vpsmgr.updatePbguiAndPb8') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="run-vps-command" data-command="vps-update-pbgui-runtime" @click="runVpsCommand('vps-update-pbgui-runtime')">{{ t('vpsmgr.updatePbguiAndRuntime') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="run-vps-command" data-command="vps-update-runtime" @click="runVpsCommand('vps-update-runtime')">{{ t('vpsmgr.updateRuntimeByProfile') }}</Button><Button v-if="!isMasterContext && hasRuntime('pb7')" type="button" variant="default" data-action="run-vps-command" data-command="vps-update-pb7" @click="runVpsCommand('vps-update-pb7')">{{ t('vpsmgr.updatePb7') }}</Button><Button v-if="!isMasterContext && hasRuntime('pb8')" type="button" variant="default" data-action="run-vps-command" data-command="vps-update-pb8" @click="runVpsCommand('vps-update-pb8')">{{ t('vpsmgr.updatePb8') }}</Button><Button v-if="isMasterContext" type="button" variant="default" data-action="run-master-update" @click="runMasterUpdate">{{ t('vpsmgr.updateLinux') }}</Button><Button v-if="isMasterContext" type="button" variant="default" @click="runMasterReboot">{{ t('vpsmgr.reboot') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="run-vps-command" data-command="vps-update-linux" @click="runVpsCommand('vps-update-linux')">{{ t('vpsmgr.updateLinux') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="run-vps-command" data-command="vps-reboot" @click="runVpsCommand('vps-reboot', false)">{{ t('vpsmgr.reboot') }}</Button><Button v-if="!isMasterContext" type="button" variant="danger" @click="requestPurge">{{ t('vpsmgr.purgeVpsInstall') }}</Button><Button type="button" variant="default" data-action="open-view" :data-view="isMasterContext ? 'master-ufw' : 'vps-ufw'" @click="openUfw">{{ t('vpsmgr.ufwLabel') }}</Button></div><template v-if="!isMasterContext"><h3>{{ t('vpsmgr.configuration') }}</h3><div class="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2.5 max-[680px]:grid-cols-1"><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsIpv4') }}<Input data-field="vps-ip" :model-value="vpsForm.ip" @update:model-value="setVpsField('ip', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserName') }}<Input data-field="vps-user" :model-value="vpsForm.user" @update:model-value="setVpsField('user', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.installPathLabel') }}<Input data-field="vps-install-dir" :model-value="vpsForm.install_dir" @update:model-value="setVpsField('install_dir', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserPassword') }}<Input type="password" data-field="vps-user-password" :model-value="vpsForm.user_pw || secretFields.user_pw || ''" @update:model-value="setVpsField('user_pw', $event)" /><Button type="button" variant="default" size="sm" data-action="reveal-secret" data-field="user_pw" class="justify-self-start" @click="revealSecret('user_pw')">{{ t('vpsmgr.reveal') }}</Button></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.swap') }}<Input data-field="vps-swap" :model-value="vpsForm.swap" @update:model-value="setVpsField('swap', $event)" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.firewallSshPort') }}<Input data-field="vps-firewall-port" type="number" :model-value="vpsForm.firewall_ssh_port" @update:model-value="setVpsField('firewall_ssh_port', numberValue($event))" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.allowedSshIps') }}<Input data-field="vps-firewall-ips" :model-value="vpsForm.firewall_ssh_ips" @update:model-value="setVpsField('firewall_ssh_ips', $event)" /></label><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="vps-runtime-profile-label">{{ t('vpsmgr.runtimeProfile') }}</span><SelectRoot :model-value="vpsForm.runtime_profile" @update:model-value="setVpsField('runtime_profile', $event)"><SelectTrigger data-field="vps-runtime-profile" aria-labelledby="vps-runtime-profile-label"><span>{{ vpsForm.runtime_profile === 'pb8' ? 'PB8 Live only' : vpsForm.runtime_profile === 'pb7_pb8' ? 'PB7 + PB8' : 'PB7' }}</span></SelectTrigger><SelectContent><SelectItem value="pb7">PB7</SelectItem><SelectItem value="pb8">PB8 Live only</SelectItem><SelectItem value="pb7_pb8">PB7 + PB8</SelectItem></SelectContent></SelectRoot></div></div><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="info" data-action="save-vps-config" @click="saveVpsConfig">{{ t('vpsmgr.save') }}</Button><Button type="button" variant="default" data-action="browse-files" @click="browseFiles">{{ t('vpsmgr.browse') }}</Button><Button type="button" variant="default" data-action="open-view" data-view="vps-setup" @click="openView('vps-setup')">{{ t('vpsmgr.setup') }}</Button></div></template><h3>{{ t('vpsmgr.instances') }}</h3><div class="overflow-auto"><table class="manager-table w-full border-collapse text-[0.78rem]"><thead><tr><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.bot') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.status') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">CPU</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">RAM</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">PNL</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.errors') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.tracebacks') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.action') }}</th></tr></thead><tbody><tr class="cursor-pointer" v-for="item in monitorItems" :key="`${item.pb_version || '7'}-${item.name}`"><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(item.name) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(item.status || (item.running ? 'running' : 'stopped')) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ formatPercent(item.cpu) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ formatBytes(item.memory_mb * 1024 * 1024) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(item.pnl_today) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(item.errors_today) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(item.tracebacks_today) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top"><Button type="button" variant="default" size="sm" data-action="bot-metric" :data-bot="historyBotName(item)" @click="openBotMetric(item)">{{ t('vpsmgr.history') }}</Button><Button type="button" variant="default" size="sm" class="ml-1" data-action="bot-log-matches" data-kind="tracebacks" :data-bot="item.name" @click="openBotLogMatches(item, 'tracebacks')">{{ t('vpsmgr.tracebacks') }}</Button><Button type="button" variant="default" size="sm" class="ml-1" data-action="bot-log-matches" data-kind="errors" :data-bot="item.name" @click="openBotLogMatches(item, 'errors')">{{ t('vpsmgr.errors') }}</Button></td></tr></tbody></table></div><h3>{{ t('vpsmgr.log') }}</h3><pre class="m-0 overflow-auto whitespace-pre-wrap break-words rounded-[5px] bg-page p-2.5 font-mono text-[0.75rem] leading-[1.4] text-primary min-h-[220px] max-h-[56vh]">{{ detail.log_preview?.content || t('vpsmgr.noLogFileFound') }}</pre></template></div></article></section>

        <section v-else-if="view === 'vps-setup'" class="grid gap-3.5 grid-cols-[repeat(auto-fit,minmax(340px,1fr))] max-[900px]:grid-cols-1"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ hostname }} — {{ t('vpsmgr.setup') }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="read-vps-settings" @click="readVpsSettings">{{ t('vpsmgr.readVpsSettings') }}</Button><Button type="button" variant="default" data-action="preview-systemd-migration" @click="send({ cmd: 'preview_vps_systemd_migration', hostname, form: { ...vpsForm } })">{{ t('vpsmgr.systemdMigration') }}</Button><Button type="button" variant="default" data-action="open-package-updates" @click="openPackageUpdates">{{ t('vpsmgr.packageUpdatesLabel') }}</Button></div></div><div class="p-3.25 grid gap-2.5"><JsonViewer :data="detail?.progress || {}" /><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="warning" data-action="setup-vps" @click="setupVps">{{ t('vpsmgr.setup') }}</Button><Button type="button" variant="info" data-action="save-vps-config" @click="saveVpsConfig">{{ t('vpsmgr.save') }}</Button><Button type="button" variant="default" data-action="cluster-onboard" @click="startClusterOnboard">{{ t('vpsmgr.addToCluster') }}</Button></div></div></article><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.preflight') }}</span></div><div class="p-3.25"><div v-for="(value, key) in detail?.status?.systemd_migration || {}" :key="String(key)" class="rounded-md bg-card p-2.25"><div class="text-[0.75rem] text-secondary">{{ key }}</div><div class="mt-0.75 text-[1.08rem] font-bold [overflow-wrap:anywhere]">{{ display(value) }}</div></div></div></article></section>

        <section v-else-if="view === 'vps-task-log' || view === 'master-task-log'" class="grid gap-3.5"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.taskLogLabel') }}</span><Button type="button" variant="default" @click="setContext(isMasterContext ? 'master' : 'vps', hostname)">{{ t('vpsmgr.backToHost') }}</Button></div><div class="p-3.25"><div id="vps-manager-log-viewer" class="min-h-[360px] overflow-hidden rounded-md border border-border-default bg-page"></div><pre v-if="!sharedLogViewer" class="m-0 overflow-auto whitespace-pre-wrap break-words rounded-[5px] bg-page p-2.5 font-mono text-[0.75rem] leading-[1.4] text-primary min-h-[220px] max-h-[56vh]">{{ detail?.progress?.update_log || detail?.progress?.setup_log || detail?.progress?.init_log || detail?.progress?.log || t('vpsmgr.noLogFileFound') }}</pre></div></article></section>

        <section v-else-if="view === 'vps-host-logs' || view === 'master-host-logs'" class="grid gap-3.5"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ t('vpsmgr.hostLogs') }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button v-if="!isMasterContext" type="button" variant="info" data-action="fetch-host-log" @click="fetchHostLog">{{ logLoading ? t('vpsmgr.loading') : t('vpsmgr.load') }}</Button><Button type="button" variant="default" data-action="browse-files" @click="browseFiles">{{ t('vpsmgr.browse') }}</Button><Button type="button" variant="default" data-action="open-package-updates" @click="openPackageUpdates">{{ t('vpsmgr.packageUpdatesLabel') }}</Button><Button v-if="!isMasterContext" type="button" variant="default" data-action="cluster-onboard" @click="startClusterOnboard">{{ t('vpsmgr.addToCluster') }}</Button></div></div><div class="p-3.25 grid gap-2.5"><div class="grid gap-1 text-[0.8rem] text-secondary"><span id="host-log-file-label">{{ t('vpsmgr.logFile') }}</span><SelectRoot :model-value="logFilename" @update:model-value="selectLogFile(String($event ?? ''))"><SelectTrigger aria-labelledby="host-log-file-label"><span :class="logFilename ? undefined : 'text-placeholder'">{{ logFilename }}</span></SelectTrigger><SelectContent><SelectItem v-for="file in detail?.logfiles || []" :key="file" :value="file">{{ file }}</SelectItem></SelectContent></SelectRoot></div><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.sizeKb') }}<Input type="number" v-model.number="logSizeKb" /></label><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox v-model="logReverse" /> {{ t('vpsmgr.reverseLog') }}</label><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox v-model="logDebug" /> {{ t('vpsmgr.debug') }}</label><div id="vps-manager-log-viewer" class="min-h-[360px] overflow-hidden rounded-md border border-border-default bg-page"></div><pre v-if="!sharedLogViewer" class="m-0 overflow-auto whitespace-pre-wrap break-words rounded-[5px] bg-page p-2.5 font-mono text-[0.75rem] leading-[1.4] text-primary min-h-[220px] max-h-[56vh]">{{ logContent || t('vpsmgr.noLogFileFound') }}</pre></div></article></section>

        <section v-else-if="view === 'vps-ufw' || view === 'master-ufw'" class="grid gap-3.5 grid-cols-[repeat(auto-fit,minmax(340px,1fr))] max-[900px]:grid-cols-1"><article class="mb-3.5 min-w-0 overflow-hidden rounded-[9px] border border-border-default bg-panel"><div class="flex items-center justify-between gap-2.5 border-b border-border-default bg-panel/46 px-3.25 py-2.75"><span class="font-bold">{{ currentUfwHost }} — {{ t('vpsmgr.ufwLabel') }}</span><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="ufw-read" @click="readUfw">{{ t('vpsmgr.load') }}</Button><Button type="button" variant="default" data-action="ufw-preview" @click="previewUfw">{{ t('vpsmgr.preview') }}</Button><Button type="button" variant="info" data-action="ufw-apply" @click="applyUfw">{{ t('vpsmgr.applyChanges') }}</Button></div></div><div class="p-3.25 grid gap-2.5"><label class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox v-model="ufw.enabled" /> {{ t('vpsmgr.enabled') }}</label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.sudoPassword') }}<Input data-field="ufw-sudo-password" type="password" v-model="ufw.sudoPw" /></label><div class="grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-2.5 max-[680px]:grid-cols-1"><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.port') }}<Input data-field="ufw-port" v-model="ufw.form.port" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.from') }}<Input data-field="ufw-from" v-model="ufw.form.from" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.comment') }}<Input data-field="ufw-comment" v-model="ufw.form.comment" /></label></div><Button type="button" variant="default" data-action="ufw-add-rule" @click="addUfwRule">{{ t('vpsmgr.addRule') }}</Button><table class="manager-table w-full border-collapse text-[0.78rem]"><thead><tr><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.delete') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">#</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.action') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.to') }}</th><th class="sticky top-0 z-[1] border-b-2 border-border-default bg-card px-1.75 py-2 text-left align-top text-primary whitespace-nowrap cursor-default">{{ t('vpsmgr.from') }}</th></tr></thead><tbody><tr class="cursor-pointer" v-for="rule in ufw.rules" :key="rule.number"><td class="border-b border-border-default px-1.75 py-2 text-left align-top"><Checkbox :model-value="ufw.deleteNumbers.includes(rule.number)" @update:model-value="toggleUfwDelete(Number(rule.number))" /></td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.number) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.action) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.to || rule.port) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.from) }}</td></tr><tr class="cursor-pointer" v-for="(rule, index) in ufw.addRules" :key="`add-${index}`"><td class="border-b border-border-default px-1.75 py-2 text-left align-top">+</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">—</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.action) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.port) }}</td><td class="border-b border-border-default px-1.75 py-2 text-left align-top">{{ display(rule.from) }}</td></tr></tbody></table></div></article></section>
      </section>
    </div>

    <div v-if="modal" class="modal-backdrop p-5" :data-modal="modal" role="dialog" aria-modal="true">
      <div class="w-[min(780px,calc(100vw-40px))] max-h-[calc(100dvh-40px)] overflow-auto rounded-lg border border-border-default bg-panel p-4.5 shadow-modal">
        <div class="mb-3.5 flex items-center justify-between gap-2.5 border-b border-border-default pb-2.5"><h2 class="m-0 text-[1.05rem]">{{ modalTitle }}</h2><Button type="button" variant="default" :data-close="modal || 'modal'" @click="closeModal"><PbIcon :icon="PhX" /> {{ t('vpsmgr.close') }}</Button></div>
        <template v-if="modal === 'confirm'"><!-- manager-btn danger: inert anchor kept for the parity-test selector --><p>{{ display(modalData.message) }}</p><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="default" @click="closeModal">{{ t('vpsmgr.cancel') }}</Button><Button type="button" variant="danger" class="manager-btn danger" @click="acceptConfirm">{{ t('vpsmgr.confirm') }}</Button></div></template>
        <template v-else-if="modal === 'password' || modal === 'deploy-password'"><label>{{ t('vpsmgr.password') }}<Input data-field="deploy-password" type="password" v-model="modalData.password" /></label><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="default" @click="closeModal">{{ t('vpsmgr.cancel') }}</Button><!-- parity anchor: the outer div stays clickable because the legacy parity suite clicks [data-action="stage-deploy-host"]; @click.stop on the button prevents a double fire -->
<div data-action="stage-deploy-host" @click="executePasswordModal"><Button type="button" variant="info" data-action="password-confirm" @click.stop="executePasswordModal">{{ t('vpsmgr.confirm') }}</Button></div></div></template>
        <template v-else-if="modal === 'host-key'"><p>{{ display(modalData.error || t('vpsmgr.unknownSshHostKey')) }}</p><JsonViewer :data="{ status: modalData.status, key_type: modalData.key_type, fingerprint: modalData.fingerprint, ssh_host: modalData.ssh_host, ip: modalData.ip }" /><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="info" data-action="trust-host-key" @click="trustHostKey">{{ t('vpsmgr.trustKeyReconnect') }}</Button></div></template>
        <template v-else-if="modal === 'package-updates'"><p>{{ display(modalData.row?.hostname) }}</p><JsonViewer :data="modalData.row?.package_status || {}" /><ul><li v-for="item in modalData.packages || []" :key="item">{{ item }}</li></ul></template>
        <template v-else-if="modal === 'ufw-preview'"><JsonViewer :data="modalData.preview || modalData" /><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="info" data-action="ufw-apply-confirm" @click="closeModal(); setModal('confirm', { title: t('vpsmgr.applyUfwChangesTitle'), message: t('vpsmgr.applyUfwChanges'), action: 'apply-ufw' })">{{ t('vpsmgr.applyChanges') }}</Button></div></template>
        <template v-else-if="modal === 'existing-import'"><div class="grid gap-2.5"><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.hostname') }}<Input data-field="existing-import-hostname" v-model="existingImport.hostname" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsIpv4') }}<Input data-field="existing-import-ip" v-model="existingImport.ip" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserName') }}<Input v-model="existingImport.user" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.vpsUserPassword') }}<Input type="password" v-model="existingImport.user_pw" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.installPathLabel') }}<Input v-model="existingImport.install_dir" /></label><label class="grid gap-1 text-[0.8rem] text-secondary">{{ t('vpsmgr.sudoPassword') }}<Input type="password" v-model="existingImport.local_sudo_pw" /></label><div class="flex flex-wrap items-center justify-end gap-1.75 max-[680px]:justify-start"><Button type="button" variant="default" data-action="resolve-existing-import" @click="resolveExistingImport">{{ t('vpsmgr.resolveHost') }}</Button><Button type="button" variant="default" data-action="probe-existing-import" @click="probeExistingImport">{{ t('vpsmgr.probe') }}</Button><Button type="button" variant="info" data-action="save-existing-import" @click="saveExistingImport">{{ t('vpsmgr.save') }}</Button></div><div v-if="modalData.probe" class="manager-import-report"><div v-for="check in modalData.probe.checks || []" :key="check.label" class="mb-3 block rounded-[7px] bg-card px-3 py-2.25 whitespace-pre-line" :class="checkStatusClass(check.ok)">{{ check.label }}: {{ display(check.detail || check.ok) }}</div><div v-for="item in modalData.probe.blockers || []" :key="`blocker-${item}`" class="mb-3 block rounded-[7px] bg-card px-3 py-2.25 whitespace-pre-line border border-danger/30 text-danger-soft">{{ item }}</div><div v-for="item in modalData.probe.warnings || []" :key="`warning-${item}`" class="mb-3 block rounded-[7px] bg-card px-3 py-2.25 whitespace-pre-line">{{ item }}</div><JsonViewer :data="modalData.probe" /></div><Button v-if="modalData.probe?.needs_host_key_confirmation || ['unknown', 'mismatch'].includes(String(modalData.probe?.host_key?.status || ''))" type="button" variant="warning" data-action="accept-existing-host-key" @click="acceptExistingHostKey">{{ t('vpsmgr.trustKeyReconnect') }}</Button></div></template>
        <template v-else-if="modal === 'cluster-import'"><div v-if="modalData.loading" class="p-4.5 text-center text-secondary">{{ t('vpsmgr.loading') }}</div><template v-else-if="modalData.progress"><JsonViewer :data="modalData.progress" /></template><template v-else><label>{{ t('vpsmgr.sudoPassword') }}<Input type="password" v-model="clusterImport.local_sudo_pw" /></label><div v-for="item in clusterImport.preview?.items || []" :key="item.hostname" class="my-2 grid grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)] items-center gap-2.5 max-[680px]:grid-cols-1"><label class="flex items-center gap-1.75 text-primary"><Checkbox v-model="clusterImport.selected[item.hostname]" /> {{ item.hostname }} — {{ item.action }}</label><Input v-if="item.action === 'add' || item.action === 'update'" type="password" :placeholder="t('vpsmgr.vpsUserPassword')" v-model="clusterImport.passwords[item.hostname]" /></div><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="info" data-action="apply-cluster-import" @click="applyClusterImport">{{ t('vpsmgr.applyChanges') }}</Button></div></template></template>
        <template v-else-if="modal === 'cluster-onboard'"><JsonViewer :data="modalData.progress || clusterOnboard.job || {}" /></template>
        <template v-else-if="modal === 'history'"><LoadingSkeleton v-if="metric.loading" :label="t('common.loading')" /><ErrorState v-else-if="metric.error" :title="t('common.error')" :message="String(metric.error)" /><MetricHistoryChart v-else :data="metric.data || modalData.data || null" :label="String(metric.name || metric.bot || 'history')" /></template>
        ...
        <template v-else-if="modal === 'systemd'"><JsonViewer :data="modalData.data || {}" /><div class="mt-3.75 flex justify-end gap-1.75"><Button type="button" variant="info" @click="closeModal(); setModal('confirm', { title: t('vpsmgr.systemdMigration'), message: t('vpsmgr.applyChanges'), action: 'systemd' })">{{ t('vpsmgr.applyChanges') }}</Button></div></template>
        <template v-else-if="modal === 'files'"><div v-if="modalData.columns" class="grid gap-2.5"><label v-for="(_, key) in visibleColumns" :key="key" class="flex items-center gap-1.75 text-[0.8rem] text-primary"><Checkbox :model-value="visibleColumns[key]" @update:model-value="toggleColumn(String(key))" /> {{ key }}</label></div><div v-else class="grid gap-1.75"><Button v-if="modalData.data?.parent" type="button" variant="default" class="justify-start text-left" @click="browsePath(modalData.data.parent)">..</Button><Button v-for="entry in modalData.data?.entries || []" :key="entry.name" type="button" variant="default" class="justify-start text-left" @click="selectBrowsedPath(`${modalData.data.cwd}/${entry.name}`, entry.type)"><PbIcon :icon="entry.type === 'dir' ? PhFolder : PhFile" /> {{ entry.name }}</Button></div></template>
      </div>
    </div>
  </div>
  </AppShell>
