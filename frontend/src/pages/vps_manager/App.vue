  ...
  <div class="vps-manager flex min-h-0 flex-1 flex-col bg-page text-primary">
    ...
    <div v-if="modal" class="modal-backdrop p-5" :data-modal="modal" role="dialog" aria-modal="true">
      <div class="w-[min(780px,calc(100vw-40px))] max-h-[calc(100dvh-40px)] overflow-auto rounded-lg border border-border-default bg-panel p-4.5 shadow-modal">
        <div class="mb-3.5 flex items-center justify-between gap-2.5 border-b border-border-default pb-2.5"><h2 class="m-0 text-md">{{ modalTitle }}</h2><Button type="button" variant="default" :data-close="modal || 'modal'" @click="closeModal"><PbIcon :icon="PhX" /> {{ t('vpsmgr.close') }}</Button></div>
...
<div data-action="stage-deploy-host" @click="executePasswordModal"><Button type="button" variant="info" data-action="password-confirm" @click.stop="executePasswordModal">{{ t('vpsmgr.confirm') }}</Button></div></div></template>
        ...
        <template v-else-if="modal === 'bot-log'"><pre class="m-0 overflow-auto whitespace-pre-wrap break-words rounded-[5px] bg-page p-2.5 font-mono text-micro leading-[1.4] text-primary min-h-[220px] max-h-[56vh]">{{ (botLog.lines || []).join('\n') }}</pre></template>
