...
/* name + modified + actions (metrics add their own columns). */
...
function onParetoRowKeydown(event: KeyboardEvent, rowPath: string): void {
        ...
        <Table class="opt-table opt-table--paretos max-[800px]:min-w-[720px] select-none bg-transparent">
          ...
          <tbody ref="tbody">
            <tr
              ...
              :key="row.path"
              :data-path="row.path"
              class="pareto-row cursor-pointer outline-none"
              :class="{ selected: selected.has(row.path) }"
              :aria-selected="selected.has(row.path) ? 'true' : 'false'"
              tabindex="0"
              @click="emit('toggle', row.path)"
              @keydown="onParetoRowKeydown($event, row.path)"
              @dblclick="emit('view', row)"
            >
              ...
              <TdActions>
                <Button type="button" variant="default" size="icon" :class="iconActionClass" :title="t('v7optimize.viewJson')" :aria-label="t('v7optimize.viewJson')" data-test="pareto-view" @click.stop="emit('view', row)"><PbIcon :icon="PhFileText" :size="16" /></Button>
                <Button type="button" variant="default" size="icon" :class="iconActionClass" :title="t('v7optimize.useAsSeed')" :aria-label="t('v7optimize.useAsSeed')" data-test="pareto-seed" @click.stop="emit('seed', row)"><PbIcon :icon="PhDna" :size="16" /></Button>
                <Button type="button" variant="default" size="icon" :class="iconActionClass" v-if="!isV8" :title="t('v7optimize.convertParetoToPb8')" :aria-label="t('v7optimize.convertParetoToPb8')" data-test="pareto-migrate" @click.stop="emit('migrate', row)"><PbIcon :icon="PhArrowRight" :size="16" /></Button>
              </TdActions>
            </tr>
            <EmptyRow
              v-if="!rows.length"
