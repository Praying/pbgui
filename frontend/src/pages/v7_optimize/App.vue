  ...
  >
    ...
    <PageToolbar :label="t('editor.optimize.pageTitle')">
      <template v-if="page.panel.value === 'configs'">
        ...
        <Button type="button" variant="default" :disabled="page.selectedConfigs.value.size !== 1" @click="page.openEditor([...page.selectedConfigs.value][0])"><PbIcon :icon="PhPencilSimple" /> {{ t('v7optimize.editSelected') }}</Button>
        <Button type="button" variant="default" data-test="duplicate-selected" :disabled="page.selectedConfigs.value.size !== 1" @click="openDuplicate([...page.selectedConfigs.value][0] || '')"><PbIcon :icon="PhCopy" /> {{ t('v7optimize.duplicate') }}</Button>
        <Button type="button" variant="default" :disabled="!page.selectedConfigs.value.size" @click="queueSelected"><PbIcon :icon="PhArrowRight" /> {{ t('v7optimize.queueSelected') }}</Button>
        <Button type="button" variant="default" :disabled="!page.selectedConfigs.value.size" @click="archiveSelected"><PbIcon :icon="PhArchive" /> {{ t('v7optimize.addToArchive') }}</Button>
        <Button type="button" variant="default" v-if="!adapter.isV8" :disabled="page.selectedConfigs.value.size !== 1" @click="migrateSelected"><PbIcon :icon="PhArrowRight" /> {{ t('v7optimize.convertToPb8Optimize') }}</Button>
        <Button type="button" variant="danger" :disabled="!page.selectedConfigs.value.size" @click="askDeleteConfigs"><PbIcon :icon="PhTrash" /> {{ t('v7optimize.deleteSelected') }}</Button>
      </template>
      <template v-else-if="page.panel.value === 'queue'">
        <Button type="button" variant="danger" :disabled="!page.selectedQueue.value.size" @click="askDeleteQueue"><PbIcon :icon="PhTrash" /> {{ t('v7optimize.deleteSelected') }}</Button>
        <Button type="button" variant="default" @click="page.settingsOpen.value = true"><PbIcon :icon="PhGear" /> {{ t('v7optimize.settings') }}</Button>
      </template>
      <template v-else-if="page.panel.value === 'results'">
        <Button type="button" variant="default" data-test="result-paretos" :disabled="!selectedResultCapabilities.hasPareto" @click="openResult(selectedResult)"><PbIcon :icon="PhChartBar" /> {{ t('v7optimize.paretos') }}</Button>
        <Button type="button" variant="default" :disabled="!selectedResultCapabilities.hasPareto" @click="openParetoExplorer()"><PbIcon :icon="PhTarget" /> {{ t('v7optimize.paretoExplorer') }}</Button>
        ...
        <Button type="button" variant="default" :disabled="!selectedResultCapabilities.hasPareto" @click="runSelectedResult('continue')"><PbIcon :icon="PhPlant" /> {{ t('v7optimize.continueOptimize') }}</Button>
        <Button type="button" variant="default" v-if="adapter.isV8" :disabled="!selectedResultCapabilities.resumable" @click="runSelectedResult('resume')"><PbIcon :icon="PhArrowsClockwise" /> {{ t('v7optimize.resumeCheckpoint') }}</Button>
        <Button type="button" variant="default" data-test="result-config" :disabled="!selectedResultCapabilities.hasConfig" @click="runSelectedResult('config')"><PbIcon :icon="PhFileText" /> {{ t('v7optimize.configDraft') }}</Button>
        <Button type="button" variant="danger" :disabled="!page.selectedResults.value.size" @click="askDeleteResults"><PbIcon :icon="PhTrash" /> {{ t('v7optimize.deleteSelected') }}</Button>
      </template>
      <template v-else>
        <Button type="button" variant="default" :disabled="!page.selectedResultPath.value" @click="openParetoExplorer()"><PbIcon :icon="PhTarget" /> {{ t('v7optimize.paretoExplorer') }}</Button>
        <Button type="button" variant="default" data-test="backtest-paretos" :disabled="!page.selectedParetos.value.size" @click="safely(backtestSelectedParetos)"><PbIcon :icon="PhArrowsClockwise" /> {{ t('v7optimize.backtest') }}</Button>
        <Button v-if="adapter.isV8" type="button" variant="default" data-test="holdout-paretos" :disabled="!page.selectedParetos.value.size || (holdoutRequiresSweepPlan && page.paretoMeta.value.sweep_cycles?.enabled !== true)" @click="safely(holdoutSelectedParetos)"><PbIcon :icon="PhHourglass" /> {{ t('v7optimize.validate') }}</Button>
        <Button type="button" variant="default" :disabled="!page.selectedParetos.value.size" @click="seedSelectedParetos"><PbIcon :icon="PhDna" /> {{ t('v7optimize.seedSelected') }}</Button>
        <Button type="button" variant="default" :disabled="!page.selectedResultPath.value" @click="runSelectedResult('continue')"><PbIcon :icon="PhFolderOpen" /> {{ t('v7optimize.seedWholeResult') }}</Button>
      </template>
      ...
      <template v-else>
        <section v-if="page.panel.value === 'configs'" class="opt-panel-view flex min-h-0 flex-col h-full"><header class="opt-panel-heading mb-2 flex items-start justify-between gap-4"><div><h1 class="m-0 text-xl">{{ panelTitle }}</h1><p class="mt-0.5 text-xs text-secondary">{{ panelSubtitle }}</p></div></header><ConfigsPanel :is-v8="adapter.isV8" :rows="page.filteredConfigs.value" :selected="page.selectedConfigs.value" :search="page.configSearch.value" :sort="page.configSort.value" @update:search="page.configSearch.value = $event" @toggle="(name) => toggle('configs', name)" @create="page.openEditor()" @edit="page.openEditor" @duplicate="openDuplicate" @sort="(key: string) => sortPanel('configs', key)" @select-all="selectVisible('configs')" @clear-selection="clearVisible('configs')" @select-range="(paths, selected) => page.setSelection('configs', paths, selected)" /></section>
        <section v-else-if="page.panel.value === 'queue'" class="opt-panel-view flex min-h-0 flex-col h-full"><header class="opt-panel-heading mb-2 flex items-start justify-between gap-4"><div><h1 class="m-0 text-xl">{{ panelTitle }}</h1><p class="mt-0.5 text-xs text-secondary">{{ panelSubtitle }}</p></div></header><QueuePanel :rows="page.filteredQueue.value" :selected="page.selectedQueue.value" :search="page.configSearch.value" :sort="page.queueSort.value" @update:search="page.configSearch.value = $event" @toggle="(filename) => toggle('queue', filename)" @action="runQueueAction" @edit="page.openQueueConfig" @log="openQueueLog" @move="(filename, delta) => safely(() => page.moveQueue(filename, delta))" @sort="(key: string) => sortPanel('queue', key)" @select-all="selectVisible('queue')" @clear-selection="clearVisible('queue')" @select-range="(paths, selected) => page.setSelection('queue', paths, selected)" @reorder="(filenames) => safely(() => page.reorderQueue(filenames))" @go-to-configs="page.setPanel('configs')" /></section>
...
/* Table interaction states ported from styles/optimize.css — row tinting
   ...
   class is shared by the Queue, Results, and Paretos panels, so these live in an
   unscoped block. */
