    ...
    <div id="sidebar" ref="sidebar">
      <div id="sidebar-sticky">
        <div id="sidebar-header">
          ...
          <div class="sb-meta">
            <span class="sb-count" id="sb-auth-state">{{ store.summaryView.value.sbAuth }}</span>
            <span class="sb-count" id="sb-ready-state">{{ store.summaryView.value.sbPb7 }}</span>
            <span class="sb-count" id="sb-pb8-state">{{ store.summaryView.value.sbPb8 }}</span>
          </div>
        </div>
    ...
    <div id="main-content">
      ...
      <section id="section-overview" class="page-section" data-section="overview" :hidden="store.activeSection.value !== 'overview'">
        <div class="panel hero-panel">
          <div class="eyebrow">{{ t('misc.welcome.pbguiWelcome') }}</div>
          <div class="hero-grid">
            <div>
              <div class="meta-strip">
                ...
                <div class="meta-pill"><strong>{{ t('misc.welcome.api') }}</strong> <span id="meta-origin">{{ apiOriginText }}</span></div>
              </div>
            </div>
            <div class="summary-grid">
              ...
              <div class="summary-card">
                ...
                <strong id="summary-identity">{{ store.summaryView.value.identity }}</strong>
        ...
        <div class="panel">
          ...
          <div class="status-list" id="status-list">
            <div v-for="row in store.statusRows.value" :key="row.label" class="status-row">
              <div class="status-label">{{ row.label }}</div>
              <div class="status-value">{{ row.state }}</div>
      ...
      <section id="section-setup" class="page-section" data-section="setup" :hidden="store.activeSection.value !== 'setup'">
        <div class="section-heading">
          <span class="section-kicker">{{ t('misc.welcome.configuration') }}</span>
          <p class="section-copy">{{ t('misc.welcome.configurationCopy') }}</p>
        </div>
        <div class="panel">
      ...
      <!-- Password -->
      <section id="section-password" class="page-section" data-section="password" :hidden="store.activeSection.value !== 'password'">
        <div class="section-heading">
          ...
          <div class="field-grid">
            <div class="field">
              ...
              <div class="pw-wrap">
                ...
                <button type="button" class="pw-eye-btn" tabindex="-1" :title="t('misc.welcome.showHideCurrentPassword')" @click="currentPwVisible = !currentPwVisible">{{ currentPwVisible ? '🙈' : '👁' }}</button>
              </div>
            </div>
            <div class="field">
              ...
              <div class="pw-wrap">
                ...
                <button type="button" class="pw-eye-btn" tabindex="-1" :title="t('misc.welcome.showHideNewPassword')" @click="newPwVisible = !newPwVisible">{{ newPwVisible ? '🙈' : '👁' }}</button>
              </div>
            </div>
          </div>
          <div class="actions">
            <button id="change-password-btn" class="btn btn-secondary" @click="store.changePassword()">{{
              store.authDisabled.value ? t('misc.welcome.enablePasswordAuthentication') : t('misc.welcome.changePassword')
            }}</button>
            <button v-if="!store.authDisabled.value" id="disable-auth-btn" class="btn btn-danger" type="button" @click="onDisableAuth()">{{ t('misc.welcome.disableAuthentication') }}</button>
          </div>
        </div>
      </section>
    </div>
