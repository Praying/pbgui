...
export function installDatePicker(): void {
  ...
  function _el(): HTMLElement {
    ...
    if (!e) {
      ...
      e.id = '__dp';
      e.style.cssText =
        'display:none;position:fixed;z-index:99999;background:var(--bg2,#1e2030);border:1px solid var(--border,#444);border-radius:8px;padding:10px 12px;box-shadow:0 6px 24px rgba(0,0,0,.7);user-select:none;min-width:220px;color-scheme:dark';
  ...
  function _render(): void {
    ...
    void prevMonth;
    void nextMonth;
    const btnS = 'background:none;border:none;color:var(--text,#ccc);cursor:pointer;font-size:1.3em;padding:0 6px;line-height:1';
    const ctlS =
      'background:var(--bg3,#2a2d40);border:1px solid var(--border,#444);color:var(--text,#ccc);border-radius:4px;padding:2px 6px;font-size:var(--fs-xs);cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:8px';
    let h =
    ...
    h +=
      '</div><div style="margin-top:8px;display:flex;justify-content:space-between"><button type="button" class="dp-foot" onclick="window.__dp.hide()" style="background:none;border:none;color:var(--text-dim,#888);cursor:pointer;font-size:var(--fs-xs);padding:2px 6px;border-radius:4px">' +
      t('common.close') +
      '</button><button type="button" class="dp-foot" onclick="window.__dp.today()" style="background:none;border:none;color:var(--accent,#4a90d9);cursor:pointer;font-size:var(--fs-xs);padding:2px 6px;border-radius:4px">' +
      t('v7explore.today') +
      '</button></div>';
    e.innerHTML = h;
    _reposition();
