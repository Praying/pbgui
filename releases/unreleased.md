...
- 保留 Vue 3 + Tailwind CSS v4.3 页面架构，将 ChatGPT 配置选择、账户使用限额、Vast 自动租赁与最大租赁数适配到对应 Vue 页面；遗留页面继续作为兼容回退。
- 保留共享导航的 AI 使用限额加载与服务重启代际保护，并同步前端资源版本、帮助文档和未发布变更记录。
...
- 保留当前 Vue 3 + Tailwind CSS v4.3 页面架构；Vast Vue 面板支持选定报价的明确租赁、Worker 确认的租赁截止时间调整，PB8 Suite 编辑器支持训练/留出窗口的可视化编辑与验证；遗留 HTML 仅作为兼容回退。
- 同步必要的脚本版本与 Pareto holdout 元数据兼容逻辑，避免远端发布中的场景、云队列和日志功能在 Vue 路径上缺失。
- 远端发布说明分别保存在 `releases/v2.04.3.md` 至 `releases/v2.04.6.md`，后端/API 变更及对应测试随合并纳入。

## 合并远端 main v2.04-v2.04.2 并适配 Vast GPU 云队列

- 合并远端 `main` 的 3 个发布提交，纳入 Vast GPU 云端租赁、队列、计费、配置校验、基准工具及 PB8 运行时修复。
- 保留当前 Vue 3 + Tailwind CSS v4.3 页面架构；将 Vast 账户凭据、GPU 偏好、报价查询、PB8 配置准备、云队列控制与任务恢复适配到 PB8 Optimize 的 Vue 队列面板。
- 保留遗留页面的远端队列校验、日志跟随、Pareto 验证和缓存版本更新，并合并对应的后端、测试和帮助文档。

## 合并远端 main v2.03.2-v2.03.7 并补齐 Vue 3 页面适配

- 合并远端 `main` 的 7 个发布提交，保留当前 Vue 3 + Tailwind CSS v4.3 迁移分支的页面路由与共享组件。
- 将 Balance Calculator 的配置替换确认、取消/失败/过期请求保护适配到 Vue composable，并将 Profit Sweep 的跨账户缓存概览、刷新间隔、立即刷新与账户匿名化适配到 Vue 页面。
- 保留遗留 HTML 作为 fallback，并合并远端的请求代际保护、操作恢复、日志错误处理和精确日志文件选择修复。
