...
def test_generated_pareto_scenario_inherits_base_exchanges() -> None:
    """Legacy fallback must backtest inherited generator scenarios on the base exchanges."""
    page = (ROOT / "frontend" / "v7_optimize.html").read_text(encoding="utf-8")
    function_source = _page_function(page, "applyParetoBacktestScenario")
    script = textwrap.dedent(
        f"""
        const assert = require('node:assert/strict');
        const deepClone = value => JSON.parse(JSON.stringify(value));
        {function_source}
        const result = applyParetoBacktestScenario({{
          backtest: {{
...
def test_pb8_scenario_controls_are_not_rendered_for_pb7() -> None:
    ...
    assert "PBGuiI18n.t('v7optimize.invalidPb8MarketIdentifiers')" in page
    assert "PBGuiI18n.t('v7optimize.pb8MarketResolverError', { message: error.message })" in page
    assert "PBGuiI18n.t('v7optimize.pb8MarketNotVerified')" in page
