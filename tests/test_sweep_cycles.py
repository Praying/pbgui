...
def test_sweep_plan_rejects_unbounded_or_malformed_sidecar_counts() -> None:
    """Persisted sidecar counts fail closed before allocating derived holdouts."""
    plan = build_sweep_plan(_config())
    plan.pop("holdout_scenarios")
    plan["holdout_count"] = 1_000_000_000

    assert validate_sweep_plan(plan) is None

    plan["holdout_count"] = "not-an-integer"
    assert validate_sweep_plan(plan) is None


def test_sweep_plan_requires_inherited_exchanges_and_matching_counts() -> None:
    """A persisted sweep remains one combined track with attributable windows."""
    config = _config()
    config["pbgui"]["scenario_template"]["parameters"]["exchange_mode"] = "per_exchange"
    assert build_sweep_plan(config) is None

    config = _config()
    config["backtest"]["scenarios"][0]["exchanges"] = ["bybit"]
    assert build_sweep_plan(config) is None

    config = _config()
    config["pbgui"]["scenario_template"]["parameters"]["training_windows"] = 3
    assert build_sweep_plan(config) is None

    config = _config()
    config["pbgui"]["scenario_template"]["holdout_scenarios"][0]["exchanges"] = ["bybit"]
    assert build_sweep_plan(config) is None

    plan = build_sweep_plan(_config())
    plan["holdout_count"] = 0
    assert validate_sweep_plan(plan) is None
