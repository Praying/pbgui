...
def test_main_page_renders_when_pb8_runtime_metadata_is_unavailable(monkeypatch) -> None:
    ...
    def serve_legacy_page(_page, _legacy_name, request, inject=None):
        html = inject("%%LIMITS_META%%", request) if inject is not None else "%%LIMITS_META%%"
        return optimize_v8.HTMLResponse(content=html, headers={"Cache-Control": "no-store"})

    monkeypatch.setattr(optimize_v8, "serve_vue_or_legacy_page", serve_legacy_page)
    request = Request({
        "type": "http",
        "scheme": "http",
...
def test_nested_suite_3d_plot_uses_temporary_normalized_candidates(optimize_v8_roots, monkeypatch) -> None:
    ...
    def fake_plot(stage):
        ...
        staged = json.loads((Path(stage) / "pareto" / "candidate-0.json").read_text(encoding="utf-8"))
