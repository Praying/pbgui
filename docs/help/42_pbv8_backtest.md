...
The dialog groups controls into **Queue concurrency**, **Automatic behavior**, and **HLCVS Cache Cleanup** sections. PB7/PB8 CPU limits, Autostart, and PBGui Market Data remain shared settings; cleanup options stay visibly disabled until cleanup is enabled, and **Clean Now** keeps its protected in-progress state.
...
The result table fills the available space at the bottom of Results; its filters and header remain visible while long lists scroll inside the table region. Clicking a row's chart, analysis JSON, config JSON, or image action opens the selected result in a separate wide dialog, so closing the report does not leave an expanding report below the list.
