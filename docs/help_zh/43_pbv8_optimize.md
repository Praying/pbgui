...
Vast 云端视图现在包含 GPU 报价、主机管理和性能历史。报价可按最低 TFLOPS 过滤，并显示本机记录的曾使用、可正常工作和偏好状态；物理机器可以设为偏好或屏蔽，屏蔽不会终止当前租赁。活动租赁可在 Worker 确认保护下调整预算、传输预留和任意非零整数分钟的截止时间。队列卡显示本地输入准备、镜像准备、断线重试、GPU/CPU/RAM/VRAM 和代理/精确评估吞吐。

性能历史由 API 每 60 秒采集到私有 SQLite 数据库，即使浏览器未打开也会继续。删除队列项不会删除历史。可按整行点击、键盘或拖动选择最多四条记录；只有工作负载指纹一致的记录才能进行 proxy/min 和 exact/min SVG 对比。筛选、分页、单条详情和相同工作负载过滤都只读取本地保留数据，不会创建租赁。
...
Vast 场景导出支持 Binance、Bybit、Bitget、OKX、Hyperliquid 和 KuCoin。场景可以继承基础交易所或显式选择交易所，并收集额外币种、warmup 历史和每个导出场所的市场元数据。GPU 参数覆盖只在冻结的 Worker 副本中展开；源配置保持不变。缺失 mapping、历史或不支持字段的错误会带场景序号、标签和确切字段路径显示在编辑器中。
