...
实时日志标签使用与「系统 → 日志 → 日志查看器」相同的原生 Vue 查看器，因此两个页面的外观、字号与控制项完全一致。

- **主机选择器**：`Local` 以及所有已配置的 VPS 主机
- **目标列表**：本地日志文件（含大小）；选择 VPS 主机时列出该主机的服务、运行中的机器人与额外日志文件，并按类型分组
- **行数**：200 / 500 / 1000 / 2000 / 5000 / 10000 / 25000 / Max (50,000)
- **级别过滤**：DBG / INF / WRN / ERR / CRT
- **预设**：Errors、Warnings、Errors + Warnings、Orders / Fills、Balance / PnL、Positions、Startup、Connection、Restart / Stop、Traceback；**All** 清除过滤
- **搜索**：全文高亮，配合 ▲ / ▼ 在匹配项之间跳转，以及隐藏非匹配行的 **Filter** 复选框
- **行号** 切换
- **Stream / Pause**、**Fetch**（重新订阅）与 **Clear**
- **Download**：将缓冲区内的行保存为文本文件
- **Restart**：重启所选目标对应的服务，或结束所选机器人实例；当服务被禁用或不符合预期时该按钮隐藏
- **连接徽标**：WebSocket 状态（连接中 / 已连接 / 已断开），意外断开后自动重连；会话过期会返回登录页

搜索、级别与预设过滤都在浏览器中对缓冲区内容执行，因此切换过滤不会重新向服务器获取数据。本页不再包含遗留的日志分组折叠视图。
