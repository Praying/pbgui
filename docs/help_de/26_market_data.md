...
Die URL des eingebetteten Vue-Job-Monitors trägt zusätzlich die aktuelle PBGui-Serial als Cache-Buster. Dadurch lädt das iframe nach Frontend-Updates den aktuellen `/api/jobs/main_page`-Build, und neue Aktionen wie `View` bleiben nicht an einer alten gecachten Monitor-Version hängen.
