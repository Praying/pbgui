...
_MAX_TRAINING_WINDOWS = 48
_MAX_HOLDOUT_WINDOWS = 16
...
def build_sweep_plan(config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if parameters.get("exchange_mode", "inherit") != "inherit":
        return None
    ...
    if not training_scenarios:
        return None
    if len(training_scenarios) > _MAX_TRAINING_WINDOWS:
        return None
    try:
    ...
    if len(holdouts) > _MAX_HOLDOUT_WINDOWS:
        return None
    holdout_scenarios = []
    ...
    try:
        ...
        configured_training_windows = int(parameters.get("training_windows") or len(training_scenarios))
        configured_holdout_windows = int(parameters.get("holdout_windows") or len(holdout_scenarios))
    except (TypeError, ValueError, OverflowError):
        return None
    if configured_training_windows != len(training_scenarios):
        return None
    if configured_holdout_windows != len(holdout_scenarios):
        return None
    return {
        "contract_version": 1,
...
def validate_sweep_plan(plan: object) -> dict[str, Any] | None:
    ...
    if not isinstance(plan, dict):
        return None
    training_scenarios = plan.get("training_scenarios")
    if not isinstance(training_scenarios, list) or not 1 <= len(training_scenarios) <= _MAX_TRAINING_WINDOWS:
        return None
    raw_holdouts = plan.get("holdout_scenarios")
    if not isinstance(raw_holdouts, list):
        try:
            holdout_count = int(plan.get("holdout_count") or 0)
        except (TypeError, ValueError, OverflowError):
            return None
        if not 0 <= holdout_count <= _MAX_HOLDOUT_WINDOWS:
            return None
        raw_holdouts = [{} for _index in range(holdout_count)]
    else:
        if len(raw_holdouts) > _MAX_HOLDOUT_WINDOWS:
            return None
        try:
            persisted_holdout_count = int(plan.get("holdout_count") or 0)
        except (TypeError, ValueError, OverflowError):
            return None
        if persisted_holdout_count != len(raw_holdouts):
            return None
    wrapper = {
        "pbgui": {
            "scenario_template": {
                ...
                "parameters": {
                    ...
                    "training_windows": len(training_scenarios),
                    "holdout_windows": len(raw_holdouts),
        ...
        "backtest": {"suite_enabled": True, "scenarios": training_scenarios},
    }
