...
| `docs/help_zh/` | Simplified Chinese guides |
...
### General
- **GUI language is English and Simplified Chinese** — English is the default and the fallback; the browser language selects Chinese automatically (`zh*`), and the top-bar language button switches per browser via `localStorage['pbgui-lang']`. Help guides: EN/DE/ZH.
...
- Guide coverage: every productive page registered in `FASTAPI_PAGES` must have a `GUIDE_TOPICS` entry and matching EN/DE/ZH Markdown topics. Review the relevant guides with every productive UI change and keep the parity/mapping tests passing.
...
- **Vue 3 form controls** (pages under `frontend/src/pages/`): use the shared ui/ component layer from `frontend/src/shared/components/ui/` — `Button` (CVA variants: primary/info/success/warning/danger/ghost/outline), `Input`, `Textarea`, `Label`, `Field` (label/hint/error scaffold with automatic id + aria wiring via provide/inject), `Select` (Reka UI listbox), `Slider`, `Switch`, `Checkbox`, `RadioGroup`. They are shadcn-vue-style wrappers over Reka UI primitives, styled entirely with the existing `@theme` tokens — the same visual language as the legacy `.btn`/`.form-input` classes, which stay valid for pages not yet migrated. Do not hand-write new Tailwind control class strings or bare `<input>`/`<select>`; migrate existing ones to these components when touching a page. The `cn()` class-merge helper lives in `frontend/src/shared/lib/utils.ts`.
- **Typography**: one type ladder lives in `frontend/src/styles/tailwind.css` (`@theme`) and is the only sanctioned source of font sizes. Ten steps, px-based, each with a bound line height: `text-micro` 11, `text-xs` 12, `text-compact` 13, `text-sm` 14, `text-base` 15, `text-md` 16, `text-lg` 19, `text-xl` 23, `text-2xl` 26, `text-3xl` 34. Pick the step whose role matches (micro = uppercase micro labels/kbd; xs = captions, dense cells, table headings; compact = dense body, field text, menu rows; sm = body; base = control values; md = panel titles; lg = section headings; xl = page titles; 2xl = metric numerals; 3xl = display numerals). Values are px, not rem: the base layer pins `html` to 15px.
  - Never write `text-[13px]`, `text-[0.8rem]`, or a raw `font-size: <px|rem>` — those were swept once already (462 overrides across ~25 near-duplicate sizes, 13px alone 140 times). The only escapes are `font-size` in `em` (relative to its own context) and `clamp()` (intentionally fluid — snap the bounds to ladder steps).
  - Line height: the per-step value is the default for single-line chrome. Wrapping copy keeps an explicit `leading-*`; do not delete one that sits on multi-line text (the frozen step defaults are Tailwind's tight 1.25-1.36 ratios, while the pages deliberately use ~1.5 on paragraphs, warning lists, tooltips and mono editors).
  - Tracking: exactly three values — `tracking-label` (uppercase micro labels, pills), `tracking-tight` (titles/headings), `tracking-display` (display numerals) — plus an explicit `tracking-normal` reset. No `tracking-[...]`, no `tracking-wide/wider/widest`.
  - Weight: exactly four values — `font-normal` / `font-medium` / `font-semibold` / `font-bold` (400/500/600/700). Never 650/750/550/800: Space Grotesk is a variable font but CJK falls back to static faces (PingFang SC, Microsoft YaHei), where those render as faux-bold.
  - A `codemod_type_scale.py` in `frontend/` implements the sweep (dry-run by default, `--write` to apply, `--report` to list the `em`/`clamp()` cases left for review). Guarded by `frontend/src/shared/components/typography-tokens.test.ts` — run `cd frontend && pnpm test` after any typography change.
...
This project is indexed by GitNexus as **pbgui** (49227 symbols, 136207 relationships, 1069 execution flows).
...
- **MUST run impact before editing.** Use `impact({target: "symbolName", direction: "upstream"})` or `node .gitnexus/run.cjs impact "symbolName" --direction upstream --repo .`; report callers, processes, and risk. Never substitute grep for graph analysis.
  - **Frequency exception (cost control):** run impact analysis once per task, covering all symbols the task will touch. Re-run it only when the task scope changes or an edit reaches a different subsystem than analyzed. Do not re-run it after every individual file edit within the same analyzed scope.
