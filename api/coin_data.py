...
def get_main_page(
    ...
    The Vue page (frontend/src/pages/coin_data) reads boot values from
    /api/boot.js at runtime; the legacy coin_data.html keeps the server-side
    %%API_BASE%%/%%VERSION%%/%%SERIAL%%/%%NAV_HASH%% injection as the
    fallback for checkouts without a frontend build.
    """

    def _inject(html: str, req: Request) -> str:
        html = render_page_urls(req, html, "/api/coin-data")
