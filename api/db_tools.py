...
def get_main_page(request: Request, session: SessionToken = Depends(require_auth)) -> FileResponse | HTMLResponse:
    """Serve the DB Tools page: built Vue entry first, legacy fallback.

    The Vue page (frontend/src/pages/db_tools) reads boot values from
    /api/boot.js at runtime; the legacy db_tools.html keeps the server-side
    placeholder injections as the fallback for checkouts without a build.
    """

    def _inject(html: str, req: Request) -> str:
        html = render_page_urls(req, html, "/api/db-tools")
    ...
    return serve_vue_or_legacy_page("db_tools", "db_tools.html", request, inject=_inject)
