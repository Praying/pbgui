...
@router.get("/hyperliquid-limits/main_page", response_class=HTMLResponse, response_model=None)
def get_hl_limits_page(
    request: Request,
    session: SessionToken = Depends(require_auth),
) -> FileResponse | HTMLResponse:
    """Serve the Vue account-limit overview with a legacy fallback."""
    del session
    ...
    def _inject(html: str, req: Request) -> str:
        ...
        html = render_page_urls(req, html, "/api/vps-manager")
        html = html.replace('"%%VERSION%%"', script_json(PBGUI_VERSION))
        return html.replace('"%%SERIAL%%"', script_json(PBGUI_SERIAL))

    return serve_vue_or_legacy_page("hl_limits", "hl_limits.html", request, inject=_inject)
