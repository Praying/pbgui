...
def get_main_page(request: Request, session: SessionToken = Depends(require_auth)):
    """Serve the built Vue 3 Profit Sweep page with a legacy fallback."""

    def inject_legacy_page(html: str, legacy_request: Request) -> str:
        """Fill the legacy fallback's runtime values without exposing secrets."""

        scheme = legacy_request.url.scheme
        host = legacy_request.url.hostname or "127.0.0.1"
        port = legacy_request.url.port
    ...
    response = serve_vue_or_legacy_page("profit_sweep", "profit_sweep.html", request, inject_legacy_page)
    response.headers["Referrer-Policy"] = "no-referrer"
    return response


@router.get("/transfers/main_page")
def get_transfers_main_page(request: Request, session: SessionToken = Depends(require_auth)):
    """Serve the built Vue Transfers page with a legacy fallback."""
    ...
    response = serve_vue_or_legacy_page("transfers", "transfers.html", request, inject_legacy_page)
    response.headers["Referrer-Policy"] = "no-referrer"
    return response
